...
import { Check, Code2, Copy, ExternalLink, Key, Layers, RefreshCw, Server, ShieldCheck, Terminal, Zap } from "lucide-react";
import Link from "next/link";
import { useState } from "react";

export default function ResellerApiDocsPage() {
  const docsUrl = "/api/v1/docs/reseller/openapi.json";
  const [activeTab, setActiveTab] = useState<"overview" | "catalog" | "purchases" | "webhooks" | "snippets">("overview");
...
echo $response;`,
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700 dark:bg-blue-950 dark:text-blue-300">
                <Code2 className="h-3.5 w-3.5" /> Developer & Reseller API Portal
              </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {activeTab === "overview" && (
          <div className="space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-xl">Authentication & Security Headers</CardTitle>
                <CardDescription>
              ...
              <CardContent className="space-y-4">
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white">Required HTTP Headers</h4>
                  <ul className="mt-3 space-y-3 text-sm text-slate-700 dark:text-slate-300">
                    <li className="flex items-start gap-2">
                      ...
                      <span>Your secret reseller API Key (e.g., <code className="font-mono text-xs">res_live_xxxxxxxx</code>).</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <code className="rounded bg-slate-200 px-2 py-0.5 font-mono text-xs font-bold text-slate-900 dark:bg-slate-800 dark:text-slate-200">
                        X-Idempotency-Key
                      </code>
                      <span>Unique random string per transaction to ensure retry safety and prevent duplicate billing.</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <code className="rounded bg-slate-200 px-2 py-0.5 font-mono text-xs font-bold text-slate-900 dark:bg-slate-800 dark:text-slate-200">
                        Content-Type
                ...
                <div className="mt-6 space-y-4">
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white">Account & Wallet Endpoints</h3>
                  <div className="rounded-lg border border-slate-200 p-4 dark:border-slate-800">
                    <div className="flex items-center gap-3">
                      <Badge className="bg-blue-600">GET</Badge>
                      <code className="font-mono text-base font-bold text-slate-900 dark:text-white">/api/v1/reseller/api/account</code>
                    </div>
                  ...
                  <div className="rounded-lg border border-slate-200 p-4 dark:border-slate-800">
                    <div className="flex items-center gap-3">
                      <Badge className="bg-blue-600">GET</Badge>
                      <code className="font-mono text-base font-bold text-slate-900 dark:text-white">/api/v1/reseller/api/wallet</code>
                    </div>
                    <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription className="mt-1">
                  List tenant products complete with reseller API pricing, product codes, and network metadata.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <h4 className="text-sm font-bold text-slate-900 dark:text-white">Query Parameters</h4>
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-xs font-mono text-slate-800 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200">
                ...
                <h4 className="text-sm font-bold text-slate-900 dark:text-white">Sample Response (200 OK)</h4>
    ...
    "pagination": { "page": 1, "perPage": 100, "total": 1 }
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription className="mt-1">
                  Inspect single product details and reseller API price by productCode.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="overflow-x-auto rounded-lg border border-slate-800 bg-slate-950 p-4 font-mono text-sm text-emerald-400">
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription className="mt-1">
                  Submit a fixed-price product purchase request using product_code and phone_number.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
  ...
  "client_reference": "ORDER_REF_99120"
                ...
                <h4 className="text-sm font-bold text-slate-900 dark:text-white">Response (200 / 202 Accepted)</h4>
                <div className="overflow-x-auto rounded-lg border border-slate-800 bg-slate-950 p-4 font-mono text-sm text-emerald-400">
                  <pre>{`{
  "success": true,
    ...
    "totalVolume": 225000
        ...
        {activeTab === "webhooks" && (
          <div className="space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-xl">Webhook Configuration & HMAC Signatures</CardTitle>
                <CardDescription>
                  Configure webhooks to receive real-time push notifications when purchase transactions reach final status.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white">Webhook Endpoints</h4>
                  <ul className="mt-3 space-y-3 text-sm">
                    <li>
                      <Badge className="bg-blue-600 mr-2">GET</Badge>
                      <code className="font-mono font-bold text-slate-900 dark:text-white">/api/v1/reseller/webhook-config</code> — Get current webhook status & URL.
                    </li>
                    <li>
                      <Badge className="bg-emerald-600 mr-2">POST</Badge>
                      <code className="font-mono font-bold text-slate-900 dark:text-white">/api/v1/reseller/webhook-config</code> — Register/update callback URL (<code className="font-mono text-xs font-bold">{"{ \"callbackUrl\": \"https://yourdomain.com/webhook\" }"}</code>).
                    </li>
                    <li>
                      <Badge className="bg-purple-600 mr-2">POST</Badge>
                      <code className="font-mono font-bold text-slate-900 dark:text-white">/api/v1/reseller/webhook-config/secret/rotate</code> — Generate new HMAC secret.
                    </li>
                  </ul>
                </div>

                <h4 className="text-sm font-bold text-slate-900 dark:text-white">Webhook Payload Example</h4>
                <div className="overflow-x-auto rounded-lg border border-slate-800 bg-slate-950 p-4 font-mono text-sm text-slate-100">
                  <pre>{`{
            ...
            <CardHeader>
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <CardDescription>Copy ready-to-run code samples for your preferred language.</CardDescription>
                </div>
